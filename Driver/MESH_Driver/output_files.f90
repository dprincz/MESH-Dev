    ...
    subroutine output_files_init(fls, shd)
        ...
        do while (ios == 0)
            ...
            select case (args(1))
                ...
                case (VN_EVPCAN, 'QFCL')
                    call output_files_append_field(fls, shd, ts, VN_EVPCAN, args, nargs, z, -1, real(ic%dts))
                case (VN_SUBLCAN, 'QFCF')
                    call output_files_append_field(fls, shd, ts, VN_SUBLCAN, args, nargs, z, -1, real(ic%dts))
                case (VN_CMAS)
                ...
                case (VN_DRAINCAN, 'ROFC')
                    call output_files_append_field(fls, shd, ts, VN_DRAINCAN, args, nargs, z, -1, real(ic%dts))

                !> Snow variables.
                case (VN_FSNO)
                    call output_files_append_field(fls, shd, ts, VN_FSNO, args, nargs, z)
                case (VN_SUBLSNO, 'QFN')
                    call output_files_append_field(fls, shd, ts, VN_SUBLSNO, args, nargs, z, -1, real(ic%dts))
                case (VN_SNO)
                ...
                case (VN_EVPSURF, 'QFG')
                    call output_files_append_field(fls, shd, ts, VN_EVPSURF, args, nargs, z, -1, real(ic%dts))
                case (VN_EVPB)
